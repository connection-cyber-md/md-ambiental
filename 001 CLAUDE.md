...
financeiro, estoque, expedição, contratos), each following the same
page → `actions.ts` (server actions) → `components/admin/*Client.tsx` structure. `app/operacional/*` and
`app/motorista/*` are still placeholders; `app/portal/*` has `coletas`/`documentos` but not the fuller
scope described in `features/portal-cliente/README.md`. Check the relevant `features/*/README.md` and
`docs/0006-ARQUITETURA-DE-DADOS-OLUC.md`'s roadmap section before assuming a module's state either way —
both directions of assumption have been wrong at different points in this repo's history.
...
- On the Postgres/RLS side, `supabase/migrations/0003_helper_functions.sql` defines `public.tenant_id()` /
  ...
  **Exception**: `supabase/migrations/20260823_rls_security_audit.sql` writes newer policies (on
  `collections`, `documents`, `financial_entries`, `contracts`) against `auth.uid()` joined through
  `public.profiles` directly, instead of the `tenant_id()`/`is_system_admin()` helpers — a second,
  parallel RLS pattern in the same schema. Know which one a policy you're touching uses before changing it.
...
copy this shape rather than introducing a new one. `schemas/*.schema.ts` (Zod, one file per domain entity)
exist for most of these entities but are **not yet wired into the server actions** — validation there is
still manual, tracked as debt in `docs/0006-ARQUITETURA-DE-DADOS-OLUC.md`.
